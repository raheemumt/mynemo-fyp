...
class Menu_generator
{
	...
	public function __construct(){
		...
		$this->ci->load->model('users/personel_model');
		$this->ci->load->library('users/auth');
		//$this->user_id 	= $this->ci->auth->user_nokp();
		//$this->is_admin = $this->ci->auth->is_admin();
	}

	public function build_menus($type=1)
	{
			$auth = $this->get_auth_permission($_SESSION['icno']);
		if(!$auth)
		{
			$auth = array(NULL);
		}
	...
	public function build_menusagent($type=1)
	{
		...
		if($type==1)
		{
			$menu = $this->ci->db->select("t1.*")
							...
							->where("t1.group_menu",$type)
							->where("t1.status",1)
							->where("t1.permission_id",5)
							//->where("t1.target","sametab")
							->group_by("t1.id")
							->order_by("t1.order","ASC")
							->get()
							->result();
	...
	public function build_menusbida($type=1)
	{
		$auth = $this->get_auth_permission($this->user_id);
		if(!$auth)
		{
		...
		if($type==1)
		{
			$menu = $this->ci->db->select("t1.*")
				...
				$html = "<ul class='breadcrumb'>
	                        <li class='".check_class('dashboard', TRUE)."'>
	                            <a href='".site_url()."' title='Utama'>
	                                <i class='icon-home'></i>
	                            </a>
	                        </li>
							<li>|</li>
							<li class='".check_class('dashboard', TRUE)."'>
	                            <a href='".site_url('bida/main_fav')."' title='Kegemaran'>
	...
	public function inbox($iduser)
	{
		...
		$menu = $this->ci->db->select("count(t1.t04_id) as jum")
							->from("{$this->x}t04_messages as t1")
							->where("t1.t04_id_user",$iduser)
							->where("t1.t04_status",NULL)
							->get()
							->result();
							if($menu->jum > 0){
							$html = $menu->jum;
