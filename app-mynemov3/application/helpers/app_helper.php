...
if(! function_exists('duit')){

    function duit($pitis)
    {
    	return number_format($pitis, 2, '.', '');
		//return money_format("%.2n",$pitis);
    }
...
if(! function_exists('tarikh')){
    ...
    function tarikh($tkh)
    {
		if ($tkh != "") {
			$tahun = explode("-", $tkh)[0];
    		if ($tahun != '0000') return date("d/m/Y", strtotime($tkh));
		}
...
if(! function_exists('tarikhmasa')){

    function tarikhmasa($tkh)
    {
    	return date("d/m/Y g:i", strtotime($tkh));
    }
