...
<nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav ml-auto">
			...
			if (!empty($covid->TKH))
			  {
				...
				echo '<div align="center">
						<small><i class="fas fa-clock" style="color:red;" aria-hidden="true" title="masa dibenarkan keluar"></i> '.$tkhwbf->WBF.'&nbsp;&nbsp;';
				echo '	<i class="fas fa-clock" style="color:green;" aria-hidden="true" title="masa sekarang"></i> '.$date.'</small>
					 </div>';
				} else {
				echo '<div align="center"><small><i class="fas fa-clock" style="color:green;" aria-hidden="true" title="masa sekarang"></i> '.$date.'</small></div>';
				}
			  } else {
			  echo "<button type='button' class='btn btn-outline-primary btn-block'>
			  <a href='/my3k/saringan/'><strong>Isi My3K </strong><br>
			  <small>(Log masuk kerja setelah My3K diisi)</small></a>  </button>
			  ";
			  } 
			  ?>
        ...
        <li class="nav-item">
            <a href="<?=site_url("logout")?>" class="nav-link btn btn-outline-dark btn-sm" title="<?php if (isset($data->NAMA)) echo $data->NAMA;?>" aria-label="Log Keluar" role="button">
