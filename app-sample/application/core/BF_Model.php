...
class BF_Model extends CI_Model
{
	...
	/** @var number sequnce name to generate auto increment number. */
    protected $sequence_auto_id = '';
    ...
    public function insert($data = null)
    {
		...
		if($this->auto_increment_id === true) {		
			$id = $this->db->query("select ".$this->sequence_auto_id.".nextval id from dual")->row()->ID;
			$data[$this->key] = $id;
		}
        ...
        if ($this->db->insert($this->table_name, $data)) {
            if ($this->return_insert_id) { 
				...
				if($this->auto_increment_id === false) {
                $id = $this->db->query("select ".$this->sequence_auto_id.".currval id from dual")->row()->ID; 
				}
    ...
    public function delete_where($where = null)
    {
        ...
        if ($this->soft_deletes !== true) {
			...
			$this->db->set($this->deleted_on_field, "SYSDATE", FALSE);
            if ($this->log_user === true) {
     ...
     */
	 /*
		Arg 1: Value (Drop down Option Text)
		Arg 2: Key (Drop Down Option Value)
		Arg 3: Where Statement
		Arg 4: Sorting
	 */
    public function format_dropdown($key="", $value="", $where="", $sorting="")
    {
        $args = func_get_args();
        if (count($args) == 2) {
            // list($key, $value) = $args;
    ...
    public function format_dropdown_lookup()
    {
        $args = func_get_args();
        if (count($args) == 2) {
            list($key, $value) = $args;
    ...
    public function created_on($row)
    {
        if (! array_key_exists($this->created_field, $row)) {
			...
			//oracle by rohing
            $this->db->set($this->created_field, "SYSDATE", FALSE);
    ...
    public function modified_on($row)
    {
        if (! array_key_exists($this->modified_field, $row)) {
            ...
            //oracle by rohing			
			$this->db->set($this->modified_field, "SYSDATE", FALSE);
        }
	...
	public function get_sequence_id($sequence_auto_id)
	{
		$id = $this->db->query("select ".$sequence_auto_id.".nextval id from dual")->row()->ID;
